...
export default class LiveProvider extends Component {
    ...
    UNSAFE_componentWillMount() {
        const { code, scope, transformCode, noInline, modules } = this.props;

        this.transpile({ code, scope, transformCode, noInline, modules });
    }
    ...
    componentDidUpdate({
        ...
        const { code, scope, noInline, transformCode, modules } = this.props;
        if (
            code !== prevCode ||
            scope !== prevScope ||
            noInline !== prevNoInline ||
    ...
    onChange = code => {
        const { scope, transformCode, noInline, modules } = this.props;
        this.transpile({ code, scope, transformCode, noInline, modules });
    };
    ...
    transpile = ({ code, scope, transformCode, noInline = false, modules }) => {
